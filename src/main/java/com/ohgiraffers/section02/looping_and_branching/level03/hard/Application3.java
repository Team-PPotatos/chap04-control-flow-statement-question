...
public class Application3 {
	...
	public static void main(String[] args) {
		...
		for (char c : str.toCharArray()) {
			if (c < 'A' || (c > 'Z' && c < 'a') || c > 'z') {
				System.out.print("영문자가 아닌 문자가 포함되어 있습니다.");
				return;
			}
		}

		System.out.print("문자 입력 : ");
		char word = sc.nextLine().charAt(0);	// 문자형으로 입력받기

		int counter = 0;

		for (char x : str.toCharArray()) {
				counter += (x == word) ? 1 : 0;	// 지금 비교하고 있는 문자가 입력받은 문자와 같다면 +1
		}

		System.out.println("포함된 횟수 : " + counter + "개");
	}
}
